...
public class Player implements Observer {
            ...
            if(ge.type == 1) {
                ...
                if(e.getKeyChar() == this.fireKey)
                {
                    ...
                    if(this.fireCounter > this.fireRefractoryPeriod)
                    {
                        ...
                        if(this.powerupCounter > 0)
                        {
                            ...
                            ((Wingman)this.workingApplet).addBullet(this.x+(this.img.getWidth(null)/4), this.y-8, 0, this.bulletSpeed, 2);
                            ((Wingman)this.workingApplet).addBullet(this.x+(this.img.getWidth(null)/4)-12, this.y-12, 
                                    (int)Math.round(speedComponent*((double)this.bulletSpeed)), (int)Math.round(speedComponent*((double)this.bulletSpeed)), 2, 135);
                            ((Wingman)this.workingApplet).addBullet(this.x+(this.img.getWidth(null)/4)-5, this.y-16, 
                ...
                if(msg.equals("EnemyCollision "+this.index)) {
                    if(boom == 0) 
                    {
                        ...
                        else
                        {
                            ...
                            health -= ((Wingman)this.workingApplet).damageFromRamming;
                            
                            this.boomTime = 5*7*1  + 1;
                            ((Wingman)this.workingApplet).addExplosion(new Explosion(this.workingApplet, explosionX, explosionY, 2, 5, 7, 1));
                            ((Wingman)this.workingApplet).playSound("Resources/snd_explosion2.wav");
                        }
                    }
                    boom = 1;
                ...
                if(msg.equals("ExplosionSmall "+this.index))
                {
                    ...
                    else
                    {
                        ...
                        ((Wingman)this.workingApplet).addExplosion(new Explosion(this.workingApplet, explosionX, explosionY, 1, 5, 6, 1));
                        ((Wingman)this.workingApplet).playSound("Resources/snd_explosion1.wav");
                    }
                } 
    ...
    private void blowUp()
    {
        if(this.lives > 0)
        {
            ...
            this.boom = 1;
            ((Wingman)this.workingApplet).addExplosion(new Explosion(this.workingApplet, this.x, this.y, 2, 5, 7, 3));
            this.x = this.spawnX;
            this.y = this.spawnY;     
